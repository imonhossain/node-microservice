...
Vite-specific guidance for `nx import`. For generic import issues (root deps, project references, name collisions, ESLint, frontend tsconfig base settings, `@nx/react` typings, Jest preset, non-Nx source handling), see `SKILL.md`.
...
Plugin init loads `vite.config.ts` before deps are available. **Fix**: `npm install --save-dev vite @vitejs/plugin-react` (or `@vitejs/plugin-vue`) first, then `npx nx add @nx/vite`.
...
React 18 (source) + React 19 (dest): npm may hoist mismatched `react-dom`, causing `TypeError: Cannot read properties of undefined (reading 'S')`. **Fix**: Align versions with `overrides`.
...
1. `npm install --save-dev eslint@^9 eslint-plugin-vue vue-eslint-parser @vue/eslint-config-typescript @typescript-eslint/parser @nx/eslint-plugin typescript-eslint`
...
1. Generic fixes from SKILL.md (root deps, executor paths, frontend tsconfig base settings, `@nx/react` typings)
...
1. Generic fixes from SKILL.md (stale files cleanup, rewritten scripts, target name prefixing, noEmit→composite, ESLint handling)
