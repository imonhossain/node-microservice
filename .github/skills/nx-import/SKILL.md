...
1. Install ESLint deps first: `npm install --save-dev eslint@^9 @nx/eslint-plugin typescript-eslint` (plus framework-specific plugins)
...
Install `typescript-eslint` explicitly — strict workspace hoisting won't auto-resolve this transitive dep of `@nx/eslint-plugin`.
...
**Fix**: `npm install --save-dev @nx/react`
...
3. Install test runner deps: `npm install --save-dev jest jest-environment-jsdom ts-jest @types/jest`
...
`nx import` may bring `node_modules/` (symlinks pointing to the source filesystem) and `package-lock.json` from the source. Both are stale.

**Fix**: `rm -rf imported/node_modules imported/package-lock.json imported/package.json (workspaces field) imported/.gitignore`, then `npm install`.
...
Nx uses `package.json` `"exports"` + workspace linking instead of tsconfig `"paths"`. If packages have proper `"exports"`, paths are redundant. Otherwise, update paths for the new directory structure.
